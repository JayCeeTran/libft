...
size_t	find_start(const char *str, const char *set)
{
	size_t	i;
	size_t	j;
...
size_t	find_end(const char *str, const char *set, int i)
{
	int	j;
...
char	*ft_strtrim(const char *str, const char *set)
{
	...
	copy = (char *) malloc((ft_strlen(str) - end + 1) * sizeof(char));
	if (!copy)
		return (NULL);
	while (start <= end)
	{
